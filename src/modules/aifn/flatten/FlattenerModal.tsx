...
function StylesList(props: {
  selectedStyle: FlattenStyleType | null,
  onSelectedStyle: (type: FlattenStyleType) => void
}) {
...
function FlatteningProgress(props: { llmLabel: string, partialText: string | null }) {
  return (
    <Box
      sx={{ mx: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
      <CircularProgress />
      <Typography>
...
export function FlattenerModal(props: {
  ...
  const handlePerformFlattening = React.useCallback(async (flattenStyle: FlattenStyleType) => {
    ...
    await startStreaming(llm.id, [
        ...
        {
          role: 'user',
          content: encodeConversationAsUserMessage(flattenProfile.userPrompt, messages),
        },
      ],
      user);

  }, [llm, props.conversationId, startStreaming, user]);
  ...
  return (
    ...
    >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, flexGrow: 1 }}>
          ...
          {isSuccess && !isError && (
            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1, alignItems: 'center' }}>
              ...
              <Button variant="outlined" onClick={() => setConfirmOverwrite(true)}
                      sx={{ ml: 'auto' }}>
                Replace Chat
              </Button>
              <Button variant="solid" onClick={() => handleReplaceConversation(true)}
      ...
      {confirmOverwrite && <ConfirmationModal
        open onClose={() => setConfirmOverwrite(false)}
        onPositive={() => handleReplaceConversation(false)}
