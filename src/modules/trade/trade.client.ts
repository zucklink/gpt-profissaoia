...
export function createConversationFromJsonV1(part: ExportedConversationJsonV1 & {
  tokenCount?: number
}) {
...
function pushOutcomeFromJsonV1(fileName: string, part: ExportedConversationJsonV1, outcome: ImportedOutcome) {
  ...
  if (!restored)
    outcome.conversations.push({
      success: false,
      fileName,
      error: `Invalid conversation: ${part.id}`,
    });
  else
...
export async function downloadConversation(conversation: DConversation, format: 'json' | 'markdown' | 'pdf') {
  ...
  if (format == 'json') {
    ...
    // link to begin the download
    await fileSave(blob, {
      fileName: `conversation-${fileTitle ? fileTitle + '-' : ''}${conversation.id}${extension}`,
    ...
    for (let i = 0; i < exportableConversation.messages.length; i++) {
      ...
      // comentado porque tem que implementar uma api interna no backend para buscar a imagem:

      // Exemplo no Node.js para pegar imagem no backend
      // const express = require('express');
      // const fetch = require('node-fetch');
      // const app = express();
      //
      // app.get('/imagem-proxy', async (req, res) => {
